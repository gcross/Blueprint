    ...
    ,testGroup "Blueprint.Tools" $
        ...
        [testGroup "analyzeDependenciesAndRebuildIfNecessary" $
            ...
            ,testCase "modified sources" $
                ...
                in withJobServer 1 (Map.singleton job_ids (encode (cache,()))) $ \job_server → do
                    ...
                    assertEqual
                        ...
                        (decode new_cache)
            -- @-node:gcross.20100628115452.1834:modified sources
            -- @+node:gcross.20100628115452.1844:modified explicit dependencies
            ,testCase "modified explicit dependencies" $
                let job_id = Identifier UUID.nil "job"
                    ...
                    cache =
                        ...
                        ,   cachedExplicitDependencies = []
                        ,   cachedImplicitDependencies = []
                        ,   cachedDependencyDigests = [product_digest]
                        ,   cachedProductDigests = product_digests
                        }
                    ...
                    correct_cache =
                        CachedDependencies
                        {   cachedSourceDigests = []
                        ,   cachedExplicitDependencies = [test_unresolved_dependency]
                        ,   cachedImplicitDependencies = []
                ...
                in withJobServer 1 (Map.singleton job_ids (encode (cache,()))) $ \job_server → do
                    ...
                    assertEqual
                        ...
                        (decode new_cache)
            -- @-node:gcross.20100628115452.1844:modified explicit dependencies
            -- @+node:gcross.20100628115452.1850:modified explicit dependency digests
            ,testCase "modified explicit dependency digests" $
                ...
                in withJobServer 1 (Map.singleton job_ids (encode (cache,()))) $ \job_server → do
                    ...
                    assertEqual
                        ...
                        (decode new_cache)
            -- @-node:gcross.20100628115452.1850:modified explicit dependency digests
            -- @+node:gcross.20100628115452.1852:modified implicit dependency digests
            ,testCase "modified implicit dependency digests" $
                let job_id = Identifier UUID.nil "job"
                    ...
                    cache =
                        ...
                        {   cachedSourceDigests = []
                        ,   cachedExplicitDependencies = []
                        ,   cachedImplicitDependencies = [test_unresolved_dependency]
                        ,   cachedDependencyDigests = [product_digest]
                        ,   cachedProductDigests = product_digests
                        }
                    dependency_job_id = identifier "4be9942a-adee-40a2-9914-2070ca3ae90f" "job"
                ...
                in withJobServer 1 (Map.singleton job_ids (encode (cache,()))) $ \job_server → do
                    ...
                    assertEqual
                        ...
                        (decode new_cache)
            -- @-node:gcross.20100628115452.1852:modified implicit dependency digests
            -- @+node:gcross.20100628115452.1846:modified miscellaneous information
            ,testCase "modified miscellaneous information" $
                ...
                in withJobServer 1 (Map.singleton job_ids (encode (cache,True))) $ \job_server → do
                    scanner_ignored_ref ← newIORef True
                    builder_called_ref ← newIORef False
                    digester_ignored_ref ← newIORef True
                    submitJob job_server job_ids $
                    ...
                    assertEqual
                        ...
                        (decode new_cache)
            -- @-node:gcross.20100628115452.1846:modified miscellaneous information
            -- @+node:gcross.20100628115452.1848:modified products
